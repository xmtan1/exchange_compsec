...
#define __USE_UNALIGN_TRP // Note: Usage Fault exception enabled for unaligned
                          // 16-bit/32-bit data transfers

#define __USE_dbgARM_MONITOR // Note: Let the dbgARM monitor do the USART
                             // initialization
...
static void __usart_init() {
  ...
  GPIO_InitStructure.GPIO_OType =
      GPIO_Mode_IN; // should really be GPIO_OType_PP?
...
static void __can_init() {
  ...
  CAN_FilterInitStructure.CAN_FilterMaskIdHigh =
      0x0000; // 0 in a position means ignore that bit
...
static void __sio_init() {
  ...
  //  RCC_AHB1PeriphClockCmd( RCC_AHB1Periph_GPIOB, ENABLE);  // already done
  // in __can_init()
...
#else
  DUMP("\n\rNOTE: Usage Fault exception disabled for unaligned 16-bit/32-bit "
       "data transfers!\n\r");
#endif

  //
  // FPU enabled by dbgARM monitor, in function SystemInit() called by
  // ResetHandler
