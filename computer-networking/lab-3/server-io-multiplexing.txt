...
{
    // using vetor to keep track of all pending connections
    std::vector<ConnectionData> connections;
    struct timeval connection_timeout; // timeout for select(), the man page has details
...
#	endif
    ...
    // set timeout
    connection_timeout.tv_sec = 5; // seconds
    connection_timeout.tv_usec = 500000; // microseconds
	...
	while( 1 )
	{
        ...
        int maxfd = listenfd; // keep track of the maximum fd value for select(), must be updated to track with the largest number of file descriptors.
        ...
        // for the initial, this block may do nothing
        for(auto &cd : connections){
            if (cd.sock == -1)
                continue; //skip this invalid connection(s)
...
#			endif
        ...
        for (auto it = connections.begin(); it != connections.end();){
           ...
           // try to process if the socket is readable.
           // each time, try to recieve or read till the end of the message.
           if(FD_ISSET(current.sock, &readfds) && current.state == eConnStateReceiving){
            #if VERBOSE
                printf("Socket %d readable -> process recv\n", current.sock);
            #endif
            furtherProcessing = process_client_recv(current);
           }

           if(FD_ISSET(current.sock, &writefds) && current.state == eConnStateSending){
            #if VERBOSE
                printf("Socket %d writable -> process send\n", current.sock);
            #endif
            furtherProcessing = process_client_send(current);
           }
